    ...
    public static function countErrorByApplicationId() {
        ...
        if ($application_list) {
            foreach ($application_list as $oneAppValue) {
                ...
                #获得最新统计数据
                $nums = ErrorLog::find()->where('ApplicationId=:appid AND AddDate>=:addDate', [':appid' => $oneAppValue['appname'], ':addDate' => $oneAppValue['lastupdatetime']])->count();
                #更新当前最新统计
    ...
    private static function saveErrorLogDay($str_time, $appname_list) {
        ...
        for ($i = 0; $i < $diff_day; $i++) {
            ...
            if (!empty($log_arr)) {
                ...
                $command->batchInsert(
                        ErrorLogDay::tableName(), ['ApplicationId', 'Number', 'Date', 'Updatetime'], $log_arr);
                $command->execute();
            }
    ...
    private static function saveErrorLogMonth($str_time, $appname_list) {
        ...
        for ($i = 0; $i < $diff_month; $i++) {
            ...
            if (!empty($log_arr)) {
                $command = \Yii::$app->db->createCommand();
                $command->batchInsert(
                        ErrorLogMonth::tableName(), ['ApplicationId', 'Number', 'Month', 'Updatetime'], $log_arr);
                $command->execute();
            }
